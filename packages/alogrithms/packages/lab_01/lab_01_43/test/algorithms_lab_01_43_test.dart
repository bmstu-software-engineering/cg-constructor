...
import 'package:lab_01_43/algorithm.dart';
...
void main() {
  ...
  group('AlgorithmL01V43 - Тестирование', () {
    test(
      'базовый тест с двумя треугольниками и углом между центрами вписанных окружностей',
      () {
        ...
        expect(
          ...
          8,
          reason:
              'Должно быть 8 точек (по 3 вершины для каждого треугольника + 2 центра вписанных окружностей)',
        );
        ...
        expect(
          result.markdownInfo,
          contains('### Центры вписанных окружностей'),
          reason: 'Должна быть информация о центрах вписанных окружностей',
        );
        expect(
          result.markdownInfo,
        ...
        expect(
          result.markdownInfo,
          contains(
            '- Угол между прямой через центры вписанных окружностей и осью абсцисс:',
          ),
          reason: 'Должна быть информация об угле между прямой и осью абсцисс',
        );
        expect(
          result.lines.length,
          7,
        ...
        expect(
          incenterAPoint.length,
          1,
          reason:
              'Должна быть 1 точка для центра вписанной окружности треугольника A',
        );
        expect(
          incenterBPoint.length,
          1,
          reason:
              'Должна быть 1 точка для центра вписанной окружности треугольника B',
        );
        expect(
        ...
        expect(
          ...
          1,
          reason:
              'Должна быть 1 линия, соединяющая центры вписанных окружностей',
        );
      },
    );
      ...
      when(() => model.data).thenReturn(
        AlgorithmLab0143DataModel(
          points: [
            ...
            // Треугольник B (расположен над треугольником A)
            Point(x: 0, y: 20),
            Point(x: 10, y: 20),
            Point(x: 5, y: 30),
