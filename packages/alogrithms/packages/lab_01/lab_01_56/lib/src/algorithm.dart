...
/// Алгоритм поиска треугольника минимальной площади, образованного точками так,
/// чтобы все три точки не принадлежали одному из множеств
class AlgorithmL01V56 implements Algorithm<FormsDataModel, ViewerResultModel> {
  @visibleForTesting
  const AlgorithmL01V56.fromModel(this._model);

  factory AlgorithmL01V56() =>
      AlgorithmL01V56.fromModel(DualPointSetModelImpl());
  ...
  ViewerResultModel _buildResult(
    ...
    for (final point in firstPoints) {
      // Выделяем точки, образующие треугольник минимальной площади
      final color =
...
- Площадь: **${area.toStringAsFixed(2)}** (минимальная)
