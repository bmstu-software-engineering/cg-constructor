...
class AlgorithmL01V44 implements Algorithm<FormsDataModel, ViewerResultModel> {
  ...
  static const String _circumcenterAColor =
      '#00FFFF'; // Голубой для центра описанной окружности A
  static const String _circumcenterBColor =
      '#FF00FF'; // Пурпурный для центра описанной окружности B
  ...
  ViewerResultModel calculate() {
    ...
    for (int i = 0; i < triangles.length - 1; i++) {
      for (int j = i + 1; j < triangles.length; j++) {
        ...
        try {
          // Вычисляем центры описанных окружностей
          final circumcenterI = GeometryCalculator.calculateCircumcenter(
            triangles[i],
          );
          final circumcenterJ = GeometryCalculator.calculateCircumcenter(
            triangles[j],
          );

          // Вычисляем угол между прямой, проходящей через центры, и осью ординат
          // Для этого поменяем местами x и y при вычислении угла
          final angle = GeometryCalculator.calculateAngle(
            Point(x: circumcenterI.y, y: circumcenterI.x),
            Point(x: circumcenterJ.y, y: circumcenterJ.x),
          );
          ...
          if (normalizedAngle > maxAngle) {
            ...
            circumcenterA = circumcenterI;
            circumcenterB = circumcenterJ;
          }
        } catch (e) {
  ...
  ViewerResultModel _buildResult(
    ...
    // Добавляем центры описанных окружностей
    resultPoints.add(
      Point(
        x: circumcenterA.x,
        y: circumcenterA.y,
        color: _circumcenterAColor,
        thickness: 3.0,
      ),
    );
    resultPoints.add(
      Point(
        x: circumcenterB.x,
        y: circumcenterB.y,
        color: _circumcenterBColor,
        thickness: 3.0,
      ),
    );

    // Добавляем линию, соединяющую центры описанных окружностей
    resultLines.add(
      Line(
        a: circumcenterA,
        b: circumcenterB,
...
- Центр описанной окружности треугольника A: (${circumcenterA.x.toStringAsFixed(4)}, ${circumcenterA.y.toStringAsFixed(4)})
- Центр описанной окружности треугольника B: (${circumcenterB.x.toStringAsFixed(4)}, ${circumcenterB.y.toStringAsFixed(4)})

### Итоговые результаты
- Угол между прямой через центры описанных окружностей и осью ординат: **${angleDegrees.toStringAsFixed(4)}°**

### Визуализация
- Зеленый: треугольник A
- Красный: треугольник B
- Голубой: центр описанной окружности треугольника A
- Пурпурный: центр описанной окружности треугольника B
- Синий: прямая, проходящая через центры описанных окружностей
''';
