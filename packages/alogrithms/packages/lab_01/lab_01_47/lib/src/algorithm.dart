...
/// Алгоритм поиска треугольника максимального периметра, внутри которого располагается заданная точка
class AlgorithmL01V47 implements Algorithm<FormsDataModel, ViewerResultModel> {
  @visibleForTesting
  const AlgorithmL01V47.fromModel(this._model);

  factory AlgorithmL01V47() =>
      AlgorithmL01V47.fromModel(AlgorithmL01V47DataModelImpl());

  final AlgorithmL01V47DataModelImpl _model;
  ...
  ViewerResultModel calculate() {
    ...
    // Находим треугольник с максимальным периметром
    ...
    return _buildResult(maxPerimeterTriangle!, pointB, points, maxPerimeter);
  }
...
### Координаты вершин треугольника максимального периметра (зеленый)
...
### Итоговые результаты
- Периметр треугольника: **${perimeter.toStringAsFixed(4)}**
