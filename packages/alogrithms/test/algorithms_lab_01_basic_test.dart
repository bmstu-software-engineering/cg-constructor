...
void main() {
  ...
  group('AlgorithmL01VBasic - Тестирование', () {
    test('создание модели данных', () {
      ...
      expect(dataModel.config.name, 'Lab 01, Variant 41');
      expect(dataModel.config.fields.length, 1);
      expect(dataModel.config.fields[0].id, 'points');
      expect(dataModel.config.fields[0].type, FieldType.list);
    });
    ...
    test(
      'исключение InvalidDataException при отсутствии обязательных полей',
      () {
        // Создаем модель данных
        final dataModel = algorithm.getDataModel();
